...
description: Generate copy-ready Chinese and English positive/negative prompts for engineering construction-process video clips from a natural-language stage description. First/last frame files are optional and are normally supplied later to the video model; use this skill for stage boundaries, object continuity, camera stability, action direction, and repair of observed failures. Do not use for generic cinematic prompts, frame creation, video generation, or full video editing.
metadata:
  version: "0.1"
...
Turn a user's description of one engineering construction-process clip into copy-ready prompt text. The normal input is text only: the user describes the stage, the action they want to see, and the visual effect they want to achieve. By default, write the prompt so the user can later upload corresponding first and last frames to the target video model. Missing frame files are never a prerequisite for ordinary prompt generation.
...
- A user needs a construction-process video prompt from a natural-language description, BIM, Blender, Revit, storyboard, or supplied reference frames.
- A user describes one construction stage and the intended visual effect, but has not prepared the frames yet.
...
Accept ordinary language. Do not require the user to fill a schema or upload files. The minimum useful description is:

```text
Construction stage or current state:
One primary action in this clip:
...
The user may provide only a paragraph. Platform, model, duration, aspect ratio, resolution, fps, masks, and observed failures are optional. First/last frame files, workflow JSON, seed, and case-manifest fields are not required for this mode.
...
3. **Chinese review prompt and negative prompt** when the user writes in Chinese or asks for a bilingual result.
4. **One short assumption note** only when an unstated choice materially affects the prompt.

For ordinary construction-video prompts, include the endpoint instruction directly in the positive prompt by default: the uploaded first frame is the starting state, the uploaded last frame is the target state, and only the described construction action may change between them. Omit this only when the user explicitly asks for a pure text-to-video prompt or says no first/last-frame workflow will be used. Do not require the user to send those frames to this skill.
...
1. **Read the user's description first.** Extract the construction stage, one primary action, intended visual effect, locked objects, camera behavior, action area, and forbidden later-stage content. First/last frames are optional downstream conditioning, not a prerequisite for prompt generation.
...
3. **Build the state card internally.** Separate invariant objects from changing objects. Describe the transition as an action path. If the description is underspecified, state one conservative assumption and continue when a useful prompt is still possible.
4. **Write the prompt package.** Put model-relevant camera and continuity constraints first, then concrete objects, action direction, spatial range, appearance order, target effect, and forbidden content. Use exact counts only when the user supplies them or the project specification supports them.
5. **Add endpoint language by default.** Make the positive prompt first/last-frame ready unless the user explicitly asks for pure text-to-video. The endpoint relationship should say that the uploaded first frame is the exact starting state, the uploaded last frame is the exact target state, and only the described construction action may change.
...
8. **Run a textual self-check.** Confirm that the package has one primary action, a bounded action area, explicit locked objects, a stage boundary, and no invented model or frame facts. Only perform visual QA when an output video is supplied and the user asks for it.
...
| The request is generic cinematic or unrelated to construction | Do not produce the requested unrelated prompt under this skill. State the scope boundary briefly and ask for the construction stage or action if the user wants to continue. |
...
- State the action direction and spatial range: for example, removal from the top downward inside the marked boundary, or lowering from above onto fixed anchor positions.
...
- Chinese text that lets the user verify the engineering meaning.
- Explicit invariants, intended change, spatial range, and stage boundary in the prompt or a short accompanying note.
